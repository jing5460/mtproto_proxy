...
#                              (copy to back server as ssl_dist.conf)
#   ssl_dist.<name>.conf     — ready-to-use Erlang TLS config for that front
#                              (copy to front server as ssl_dist.conf)
#
# After setup:
#   On back server:   ca.pem  back.pem  back.key  ssl_dist.conf (from ssl_dist.back.conf)
#   On each front:    ca.pem  <name>.pem  <name>.key  ssl_dist.conf (from ssl_dist.<name>.conf)
...
%% Copy this file to /etc/mtproto-proxy/dist/ssl_dist.conf on the '$NAME' server.
[
  {server,
...
case "$CMD" in
    init)
        ...
        echo "  2. Copy ssl_dist.back.conf to /etc/mtproto-proxy/dist/ssl_dist.conf"
    ...
    add-node)
        ...
        echo "  ssl_dist.$NAME.conf  ->  rename to ssl_dist.conf on that server"
        ;;
    *)
        usage
        ;;
